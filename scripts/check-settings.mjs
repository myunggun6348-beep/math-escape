...
for(const c of courses)for(const unit of ['all',...courseUnits[c]])for(const difficulty of ['basic','standard','advanced'])for(const seed of [0,1,2]){
 ...
 qs.forEach((q,i)=>{assert.equal(q.id,i);assert.ok(Number.isFinite(q.answer));assert.equal(q.difficulty,difficulty);assert.ok(q.prompt&&q.ask&&q.hint&&q.explanation.length);assert.ok(q.explanation.every(s=>typeof s==='string'));if(unit!=='all')assert.equal(q.unit,unit);questionCount++;for(const f of q.formula??[]){const html=renderToStaticMarkup(createElement(MathExpression,{source:f}));assert.ok(!/NaN|undefined|frac\{|pow\{|matrix\{/.test(html));}if(q.diagram){kinds.add(q.diagram.kind);const html=renderToStaticMarkup(createElement(MathDiagram,{diagram:q.diagram}));assert.ok(html.includes('<svg'));assert.ok(!/NaN|Infinity/.test(html));diagramCount++;}});
}
assert.equal(kinds.size,7);assert.throws(()=>validateSettings('대수','미분','basic'));assert.throws(()=>validateSettings('대수','all','invalid'));
// Verify numerical identities independently for graph-backed calculus and probability.
for(let seed=0;seed<3;seed++){
 const integrals=questions('미적분Ⅰ',3,{unit:'적분',difficulty:'standard'},seed);integrals.forEach((q,i)=>{const n=2+seed+i;assert.ok(Math.abs(q.answer-(i%2?n*n:n**3/6))<1e-9);});
 const variances=questions('확률과 통계',3,{unit:'통계',difficulty:'advanced'},seed);variances.forEach((q,i)=>{const n=2+seed+i;const mean=(n+2*(n+1)+(n+2))/4;const variance=(n*n+2*(n+1)**2+(n+2)**2)/4-mean*mean;assert.ok(Math.abs(q.answer-n*n*variance)<1e-9);});
}
async function post(body,user='teacher'){const response=await POST(new Request('https://game.test/api/game',{method:'POST',headers:{'content-type':'application/json','oai-authenticated-user-id':user},body:JSON.stringify(body)}));return{status:response.status,...await response.json()};}
