...
const cache=new Map();
export function moduleUrl(path){const file=resolve(path);if(cache.has(file))return cache.get(file);let source=readFileSync(file,'utf8');if(file.endsWith('/db/raw.ts'))source='export const db=()=>globalThis.testDb;';
